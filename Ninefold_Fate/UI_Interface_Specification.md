...
- **候選圖塊區**：位置 (140, 1620)，4個檔位，每個200×200px
  - **圖塊縮放顯示規則**：
    - 單格圖塊：原始大小，佔用整個200×200px區域
    - 多格圖塊（L型、T型、十字型等）：等比例縮小以適應200×200px容器
    - 縮放計算：以圖塊的最大寬度或高度為基準，縮放至容器80%尺寸(160×160px)
    - 縮放後圖塊居中顯示於200×200px容器內
    - 保持圖塊各格子間的相對位置和比例關係
  - 顯示圖塊形狀與屬性
  - 支援拖曳到中層的3×3戰鬥棋盤
  - 拖曳時顯示原始大小預覽
...
- 拖拽預覽縮放：1.1 倍
- **多格圖塊特殊處理**：
  - 候選區顯示：縮放至容器80%尺寸(160×160px)，居中顯示
  - 拖拽時恢復：自動放大到實際尺寸進行預覽
  - 實際尺寸計算：每格66.67px（200px÷3）符合3×3棋盤格大小
  - 多格圖塊拖拽時保持各格子間相對位置不變
...
**容器規格**：
- 標準尺寸：200×200px（與棋盤格子對齊）
- 可視區域：160×160px（容器的80%，居中顯示）
- 邊框：4px，顏色依據圖塊元素屬性
- 背景：半透明，便於識別圖塊形狀
...
**多格圖塊縮放算法**：
- **尺寸計算基準**：以圖塊實際佔用的最大寬度或高度為基準
- **縮放係數計算**：`scale = min(160 / max_width_px, 160 / max_height_px)`
- **實際格子尺寸**：在棋盤中每格為66.67px（200px÷3）
- **縮放後居中**：縮放後的圖塊在200×200px容器中水平和垂直居中

**具體縮放示例**：
- L型圖塊 (2×3格)：實際尺寸 133.34×200px → 縮放至 106.67×160px (縮放係數0.8)
- 直線圖塊 (1×3格)：實際尺寸 66.67×200px → 縮放至 53.33×160px (縮放係數0.8)
- 十字圖塊 (3×3格)：實際尺寸 200×200px → 縮放至 160×160px (縮放係數0.8)
- T型圖塊 (3×2格)：實際尺寸 200×133.34px → 縮放至 160×106.67px (縮放係數0.8)
- 2×2方塊 (2×2格)：實際尺寸 133.34×133.34px → 縮放至 106.67×106.67px (縮放係數0.8)
...
**拖拽預覽圖塊規格**：
- 尺寸：恢復到實際大小（每格66.67×66.67px）
- 透明度：90%
- 陰影：12px blur #00000050
- 整體縮放：110%（視覺強化效果）
- 各格子保持相對位置和66.67px間距

**棋盤格子對齊規則**：
- 拖拽預覽的各格子必須對齊到3×3棋盤的格子邊界
- 吸附距離：50px，超過此距離開始磁吸效果
- 多格圖塊以第一個格子（通常是左上角）為基準點進行對齊

#### 4.8.3 視覺識別輔助設計

**形狀識別輔助**：
- 每個格子使用統一的圓角：15px
- 格子間間距：2px（在候選區縮放版本中等比例縮小）
- 邊框顏色：依據元素屬性（火：#FF4444，水：#4488FF等）
- 格子填充：元素屬性顏色的70%透明度
...
- 十字型：中央顯示「+」符號
- 直線型：顯示「—」或「|」符號
- 圖標尺寸：20×20px，顏色：#FFFFFF，字體：16px Bold
