...
計畫新增設定選項：開啟後棋盤放滿時自動觸發結束回合，省去老手重複點按的步驟，增加操作流暢感。
技術上需要：① EventBus 定義 `board_completed` 訊號；② BattleBoard.on_board_completed() 發送訊號（L426 已有注釋掉的預留代碼）；③ 設定系統讀取開關值決定是否自動送出。
...
**S13** `[✅ 已修復]` **BaseStateMachine 啟動訊息標籤錯誤**
...
*影響：scripts/state_machine/BaseStateMachine.gd*
> **修復說明：** S12 修復時已連同整段 _ready() 快捷鍵說明一起移除，問題不復存在。
...
**S17** `[✅ 已修復]` **BaseSkill._get_localized_name() 應直接存取 autoload**
...
*影響：scripts/skills/BaseSkill.gd*
> **修復說明：** 改為 `ResourceManager.current_language if ResourceManager else "zh"`，與 BaseCharacter、BattleTile 寫法一致。
...
**S19** `[✅ 已解決]` **HealSkill 假設 heal() 的回傳值格式**
...
> **說明：** C01 修復時已改為透過 `EventBus.skill_effect_requested.emit()` 委派，不再直接呼叫 `target.heal()`，問題隨之消失。
...
*影響：scripts/ui/BattleBoard.gd*
> **決定：** 此訊號與「棋盤滿格自動送出」功能綁定，待該功能實作時一併處理。詳見 U20。

**S21** `[✅ 已修復]` **BattleBoard 格子編號標籤（0~8）為調試殘留**
`create_grid_layout()` 在每格加入 0~8 的數字標籤，此為調試用途，**應在正式版本中隱藏或移除**。
*影響：scripts/ui/BattleBoard.gd*
> **修復說明：** 新增 `@export var show_debug_labels: bool = false`，預設隱藏，需要時在 Inspector 手動勾選開啟。

**S22** `[✅ 已修復]` **DraggableTile.setup_base_style() 只呼叫一次**
`setup_base_style()` 只在 `_ready()` 時呼叫一次，若子類修改了樣式後（如高亮選中），**無法自動還原**基本樣式，建議提供重置方法。
*影響：scripts/ui/DraggableTile.gd*
> **修復說明：** 新增 `reset_visual()` 方法統一還原 `modulate` 與 `scale`，`set_dragging_visual(false)` 和 `set_hover_effect(false)` 改為共同呼叫 `reset_visual()`，消除重複邏輯。
...
**S24** `[✅ 已修復]` **BattleTile.setup_from_resource_manager() 可能重複初始化**
工廠方法創建後會設定資料，`_ready()` 中已呼叫一次初始化，`setup_from_resource_manager()` 再次呼叫時雖有 `if is_inside_tree()` 檢查，**仍可能造成重複初始化**，建議加入初始化狀態標記。
*影響：scripts/ui/tiles/BattleTile.gd*
> **修復說明：** 覆寫 `setup_base_style()`，只保留最小尺寸設定，跳過父類的 TypeLabel 和灰色 StyleBox。兩者本來就會被 `setup_battle_tile_style()` 立刻清除重建，覆寫後消除無謂的建立-刪除循環，最終顯示完全相同。
