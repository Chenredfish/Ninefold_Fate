...
#### 2.1.2 中層 - 統一確認區域 (390, 1000) - (690, 1300)
- **3×3 導航網格**：300×300px，九宮格居中
- **單格尺寸**：100×100px，無間距
- **中央確認格**：特殊高亮效果，"將圖塊拖放到此處"提示
- **視覺反饋**：拖放時有吸附效果和高亮顯示

#### 2.1.3 下層 - 功能圖塊區 (0, 1300) - (1080, 1920)
...
#### 2.2.2 中層 - 統一確認區域 (390, 1000) - (690, 1300)
- **3×3 確認網格**：300×300px，與主選單相同規格
- **單格尺寸**：100×100px，無間距
- **中央確認格**：顯示"拖放關卡圖塊開始遊戲"
- **確認/取消按鈕**：選中關卡後在九宮格下方顯示

#### 2.2.3 下層 - 關卡圖塊滾動區 (0, 1300) - (1080, 1920)
- **水平滾動容器**：支援右滑查看更多關卡
- **關卡圖塊**：統一尺寸 160×160px
- **解鎖狀態標示**：
  - 已完成：綠色邊框 + 星級顯示
  - 可挑戰：白色邊框 + 關卡編號
  - 未解鎖：灰色 + 鎖定圖示
- **難度選擇器**：位置 (540, 1800)，三個選項 (普通/困難/地獄)
...
#### 2.3.2 中層 - 3×3戰鬥棋盤區 (390, 1000) - (690, 1300)
**唯一的戰鬥棋盤，同時具備確認功能**：
- **3×3 戰鬥棋盤**：300×300px，與其他介面的九宮格相同規格
- **單格尺寸**：100×100px，無間距
- **功能**：
  - 候選圖塊拖放的目標區域
  - 障礙格標示、放置高亮
...
- **候選圖塊區**：4個圖塊槽位，每個 160×160px，位置 (140, 1350)
  ...
  - 支援拖曳到中層的3×3戰鬥棋盤
  - 使用後立即補充新圖塊
- **送出按鈕**：位置 (440, 1550)，200×60px
- **取消按鈕**：位置 (680, 1550)，160×60px
...
│   │       └── ElementBonus (屬性加成)
│   └── BattleBoard (中層唯一戰鬥棋盤 300x300px)
│       ├── GridContainer (3x3, 100x100px單格)
│       ├── PlacedTiles (已放置圖塊)
...
#### 2.4.2 中層 - 統一確認區域 (390, 1000) - (690, 1300)
- **3×3 行動確認網格**：300×300px，與其他介面相同規格
- **單格尺寸**：100×100px，無間距
- **中央確認格**：顯示"選擇你的行動"
- **確認提示**：拖放行動圖塊後顯示確認效果

#### 2.4.3 下層 - 行動圖塊選擇區 (0, 1300) - (1080, 1920)
...
**中層唯一戰鬥棋盤 (300×300px)**：
- **3×3戰鬥棋盤**：位置 (390, 1000)，與其他介面九宮格相同規格
- **單格尺寸**：100×100px，無間距
- **棋盤功能**：
  - 候選圖塊的拖放目標區域
...
**下層候選圖塊與控制區 (620px高)**：
- **候選圖塊區**：位置 (140, 1350)，4個檔位，每個160×160px
  - 顯示圖塊形狀與屬性
  - 支援拖曳到中層的3×3戰鬥棋盤
  - 使用後立即補充新圖塊
- **操作按鈕**：
  - 送出按鈕：位置 (440, 1550)，200×60px，背景 #FF6600
  - 取消按鈕：位置 (680, 1550)，160×60px，背景 #888888
...
- [ ] 3×3戰鬥棋盤與其他介面的九宮格規格完全一致
- [ ] 下層只有候選圖塊和送出/取消按鈕，不包含設定按鈕
- [ ] 設定按鈕固定在右上角，屬於上層區域
