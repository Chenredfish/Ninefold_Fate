...
- **容器背景**：
  ...
  - 背景色：#2a2a3e
- **圖塊通用規格**：
  - 標準尺寸：200×200px（與棋盤格子對齊，100px的2倍）
  - 圓角：20px
  - 邊框：4px
  - 圖標尺寸：80×80px，位置中央偏上 30px
  - 文字標籤：22px Bold，位置底部偏上 30px
- **控制按鈕規格**：
  - 主要按鈕：200×60px
  - 次要按鈕：80×60px
...
#### 4.5.1 圖塊通用規格
- **標準圖塊尺寸**：200×200px (大，與棋盤對齊)，100×100px (中)，80×80px (小)
- **圓角半徑**：20px (大圖塊)，10px (中圖塊)，8px (小圖塊)
- **邊框寬度**：4px (強調)，2px (正常)，1px (次要)
- **陰影效果**：6px blur，透明度 25%，顏色 #000000
...
**基礎尺寸**：
- 標準戰鬥圖塊：200×200px（棋盤格子尺寸）
- 候選區圖塊：200×200px（與棋盤格子相同）
- 迷你圖塊：200×200px（與棋盤格子對齊）

**火屬性圖塊**：
- 背景色：#FF4444
- 邊框：4px #AA0000
- 圖標位置：中央偏上 30px
- 圖標尺寸：100×100px (標準，與格子相同)
- 屬性文字：底部中央，字體 22px Bold
- 拖曳狀態：透明度 0.8，陰影 10px #FF444460
...
#### 10.1.1 功能圖標
**主選單功能圖塊圖標**：
- `sword_shield.png` - 戰鬥圖標 (80×80px)
- `puzzle_piece.png` - 構築圖標 (80×80px)  
- `gold_coins.png` - 商店圖標 (80×80px)
- `gear.png` - 設定圖標 (80×80px)

**操作圖標**：
- `retry_arrow.png` - 重試圖標 (80×80px，圓形箭頭)
- `continue_arrow.png` - 繼續圖標 (80×80px，右箭頭)
- `back_arrow.png` - 返回圖標 (80×80px，左箭頭)
- `share_icon.png` - 分享圖標 (80×80px，分享符號)

**狀態圖標**：
- `lock.png` - 鎖定圖標 (60×60px)
- `star_gold.png` - 金星 (30×30px 和 100×100px 兩種尺寸)
- `star_silver.png` - 銀星 (30×30px 和 100×100px 兩種尺寸)
- `star_empty.png` - 空星 (30×30px 和 100×100px 兩種尺寸)

#### 10.1.2 屬性圖標
**元素屬性** (60×60px 和 100×100px 兩種尺寸)：
...
- `hero凸塊_icon.png` - 總覽用小圖標 (100×100px)
