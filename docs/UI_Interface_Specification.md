...
#### 上層 - 資訊顯示區 (0, 0) - (1080, 640)
- **主選單**：遊戲標題 + 副標題
- **關卡選擇**：章節資訊 + 關卡詳細資訊
- **戰鬥界面**：敵人立繪 + HP/倒數 + 傷害/Combo顯示
- **結算畫面**：勝敗結果 + 星級評價 + 統計資料

#### 中層 - 互動九宮格區 (0, 640) - (1080, 1040)  
**統一規格**：
- 九宮格尺寸：360×360px，位置居中 (360, 640)
- 單格尺寸：120×120px，間距：0px
- 中央確認格：特殊高亮效果
- 所有介面使用相同尺寸和位置

#### 下層 - 選項圖塊區 (0, 1040) - (1080, 1920)
- **標準四圖塊**：主選單、結算畫面
- **戰鬥六圖塊**：4個候選圖塊 + 送出按鈕 + 設定按鈕
- **關卡滾動區**：水平滾動關卡圖塊 + 返回按鈕
...
#### 2.1.1 上層 - 資訊顯示區 (0, 0) - (1080, 640)
- **遊戲 LOGO**：位置 (540, 200) 中心對齊，"Ninefold Fate" 標題
- **副標題/標語**：位置 (540, 320)，例如 "選擇你的命運方塊"
- **版本資訊**：右下角顯示版本號

#### 2.1.2 中層 - 統一九宮格區域 (360, 640) - (720, 1000)
- **3×3 導航網格**：360×360px，九宮格居中
- **單格尺寸**：120×120px，無間距
- **中央確認格**：特殊高亮效果，"將圖塊拖放到此處"提示
- **視覺反饋**：拖放時有吸附效果和高亮顯示

#### 2.1.3 下層 - 功能圖塊區 (0, 1040) - (1080, 1920)
- **四個功能圖塊**：水平排列，統一尺寸 160×160px
...
MainMenu.tscn
├── CanvasLayer (背景層)
│   ├── InfoDisplayArea (上層資訊區 640px高)
│   │   ├── GameLogo
│   │   ├── SubtitleLabel
│   │   └── VersionLabel
│   └── StandardGrid (中層統一九宮格 360x360px)
│       ├── GridContainer (3x3, 120x120px單格)
│       ├── CenterSlot (目標放置區)
│       └── GridHighlight
├── CanvasLayer (互動層)
│   └── TileSelectionArea (下層圖塊區)
│       ├── FunctionTileContainer (四個功能圖塊)
│       │   ├── BattleTile (戰鬥圖塊)
│       │   ├── DeckBuildTile (構築圖塊)
│       │   ├── ShopTile (商店圖塊)
│       │   └── SettingsTile (設定圖塊)
│       └── BackButton (返回按鈕，此頁面隱藏)
└── CanvasLayer (效果層)
    ├── DragPreview
    └── TransitionEffect
```
...
#### 2.2.1 上層 - 資訊顯示區 (0, 0) - (1080, 640)
- **章節標題**：位置 (540, 100)，當前章節名稱與編號
- **章節進度條**：位置 (540, 150)，完成關卡數 / 總關卡數
- **關卡詳細資訊面板**：位置 (540, 400)，顯示選中關卡的詳細資訊
  - 關卡名稱、敵人預覽、獎勵資訊
  - 解鎖條件、建議戰力等
- **章節切換按鈕**：左右箭頭 (40, 100) 和 (1000, 100)

#### 2.2.2 中層 - 統一九宮格區域 (360, 640) - (720, 1000)
- **3×3 確認網格**：360×360px，與主選單相同規格
- **單格尺寸**：120×120px，無間距
- **中央確認格**：顯示"拖放關卡圖塊開始遊戲"
- **確認/取消按鈕**：選中關卡後在九宮格下方顯示

#### 2.2.3 下層 - 關卡圖塊滾動區 (0, 1040) - (1080, 1920)
- **水平滾動容器**：支援右滑查看更多關卡
- **關卡圖塊**：統一尺寸 160×160px
- **解鎖狀態標示**：
  - 已完成：綠色邊框 + 星級顯示
...
#### 2.3.1 上層 - 敵人資訊顯示區 (0, 0) - (1080, 640)
- **敵人立繪**：位置 (200, 320)，尺寸 200×200px，圓形遮罩
- **敵人資訊面板**：位置 (450, 200)
  - HP 血條：600×40px
  - 屬性 Icon：40×40px
  - 倒數 CD 顯示：120×120px 圓形，大字顯示剩餘回合數
- **傷害/Combo 顯示區**：位置 (450, 400)
  - 本回合傷害預覽
  - 當前 Combo 數值
  - 連擊倍率顯示
- **技能按鈕區**：位置 (800, 300)，預留英雄技能按鈕
- **關卡名稱**：位置 (540, 50)，當前關卡標題

#### 2.3.2 中層 - 統一九宮格區域 (360, 640) - (720, 1000)
**注意：戰鬥棋盤是9×9，但九宮格確認區域保持3×3統一規格**
- **3×3 確認網格**：360×360px，與其他介面相同規格
- **單格尺寸**：120×120px，無間距
- **確認區域功能**：顯示當前回合操作預覽
- **送出確認**：中央格可顯示"送出本回合"

#### 2.3.3 下層 - 戰鬥操作區 (0, 1040) - (1080, 1920)
- **候選圖塊區**：4個圖塊槽位，每個 160×160px
  - 顯示圖塊形狀與屬性
  - 支援拖曳到實際的9×9戰鬥棋盤
  - 使用後立即補充新圖塊
- **送出按鈕**：位置 (540, 1400)，280×80px
- **設定按鈕**：位置 (920, 1450)，80×80px
- **9×9實際戰鬥棋盤**：位置 (90, 1100)，900×900px
  - 障礙格標示、放置高亮
  - 已放置圖塊顯示
  - 3×3 區域標示
...
Battle.tscn
├── CanvasLayer (背景層)
│   ├── InfoDisplayArea (上層敵人資訊區 640px高)
│   │   ├── EnemyPanel
│   │   │   ├── EnemySprite (200x200px圓形)
│   │   │   ├── HPBar (600x40px)
│   │   │   ├── ElementIcon (40x40px)
│   │   │   └── CountdownLabel (120x120px圓形)
│   │   ├── DamageComboDisplay (傷害Combo顯示)
│   │   ├── SkillButtonArea (技能按鈕區)
│   │   └── LevelTitle (關卡名稱)
│   └── StandardGrid (中層統一九宮格 360x360px)
│       ├── GridContainer (3x3, 120x120px單格)
│       ├── CenterSlot (回合操作預覽)
│       └── SubmitIndicator (送出指示)
├── CanvasLayer (互動層)
│   └── TileSelectionArea (下層戰鬥操作區)
...
│       │   └── TileSlot4
│       ├── ActionButtons
│       │   ├── SubmitButton (280x80px)
│       │   └── SettingsButton (80x80px)
│       └── BattleBoard (9x9實際戰鬥棋盤 900x900px)
│           ├── GridContainer (9x9)
│           ├── PlacedTiles
│           └── HighlightOverlay
└── CanvasLayer (浮動層)
...
#### 2.4.1 上層 - 結果展示區 (0, 0) - (1080, 640)
- **勝利/失敗大字**：位置 (540, 150)，動畫效果的結果標題
- **星級評價**：位置 (540, 250)，3星評價系統
- **戰鬥統計面板**：位置 (540, 400)
  - 總回合數 vs 目標回合數
  - 最大連擊數
  - 總傷害輸出
  - 剩餘 HP（勝利時）
  - 完成時間
- **獎勵展示**：位置 (540, 550)，獲得的金幣、經驗、道具等

#### 2.4.2 中層 - 統一九宮格區域 (360, 640) - (720, 1000)
- **3×3 行動確認網格**：360×360px，與其他介面相同規格
- **單格尺寸**：120×120px，無間距
- **中央確認格**：顯示"選擇你的行動"
- **確認提示**：拖放行動圖塊後顯示確認效果

#### 2.4.3 下層 - 行動圖塊選擇區 (0, 1040) - (1080, 1920)
- **四個行動圖塊**：統一尺寸 160×160px，水平排列
...
Result.tscn
├── CanvasLayer (背景層)
│   ├── InfoDisplayArea (上層結果展示區 640px高)
│   │   ├── ResultTitle (勝敗大字)
│   │   ├── StarRating (星級評價)
│   │   ├── BattleStats (戰鬥統計面板)
│   │   │   ├── TurnsDisplay
...
**所有介面採用統一三層結構**：
- **上層資訊顯示區**：位置 (0, 0)，尺寸 1080×640px
- **中層統一九宮格**：位置 (360, 640)，尺寸 360×360px
- **下層圖塊選擇區**：位置 (0, 1040)，尺寸 1080×880px

#### 4.1.2 上層資訊顯示區詳細規格 (640px高)
- **背景色**：深藍漸層 (#1a1a2e → #16213e)
- **內容區域劃分**：
  - 標題區：(0, 0) - (1080, 200px)
  - 主要資訊區：(0, 200) - (1080, 500px)
  - 副資訊區：(0, 500) - (1080, 640px)

#### 4.1.3 中層統一九宮格規格 (360×360px)
**所有介面使用相同規格**：
- **位置**：(360, 640) 固定位置，水平居中
- **總尺寸**：360×360px
- **網格配置**：3×3，每格 120×120px
- **格子間距**：0px（緊密排列）
- **背景**：半透明黑色 (#000000AA)
- **圓角**：20px
- **邊框**：2px 實線 #444444
...
- **圖塊通用規格**：
  ...
  - 圓角：16px
  - 邊框：4px
  - 圖標尺寸：60×60px，位置中央偏上 20px
  - 文字標籤：18px Bold，位置底部偏上 20px

### 4.2 各介面具體規格

#### 4.2.1 主選單 (MainMenu) 具體佈局
**上層資訊顯示區內容**：
- **遊戲 LOGO**：位置 (540, 100) 中心對齊，字體 48px Bold #FFD700
- **副標題**：位置 (540, 180)，「選擇你的命運方塊」，20px #CCCCCC
- **版本資訊**：位置 (1000, 600)，12px #888888

**下層功能圖塊排列**：
- **四個圖塊水平居中排列**：
  - 戰鬥圖塊：(190, 1200)，背景色 #FF4444
  - 構築圖塊：(370, 1200)，背景色 #4488FF
  - 商店圖塊：(550, 1200)，背景色 #FFD700
  - 設定圖塊：(730, 1200)，背景色 #888888

#### 4.2.2 關卡選擇 (LevelSelection) 具體佈局
**上層資訊顯示區內容**：
- **章節標題**：位置 (540, 50)，28px Bold #FFFFFF
- **章節進度條**：位置 (540, 100)，920×20px，背景 #333333，填充 #44AA44
- **章節切換按鈕**：左 (40, 100)，右 (1000, 100)，60×40px
- **關卡詳細資訊面板**：位置 (540, 350)，顯示選中關卡資訊
  - 關卡名稱、敵人預覽、獎勵資訊
  - 解鎖條件、建議戰力
  - 背景：半透明白色 #FFFFFF80，圓角 20px

**中層九宮格功能**：
- 顯示「拖放關卡圖塊開始遊戲」提示
- 選中關卡後顯示確認/取消按鈕
- 確認按鈕：位置九宮格下方 (540, 1020)，200×50px

**下層關卡圖塊滾動區**：
- **滾動容器**：位置 (40, 1100)，1000×700px
- **可視區域**：1000×200px，支援水平滾動
- **關卡圖塊規格**：
  - 尺寸：160×160px（統一標準）
  - 間距：20px
...
- **返回主選單按鈕**：位置 (80, 1750)，120×80px

#### 4.2.3 戰鬥場景 (Battle) 具體佈局
**上層敵人資訊顯示區內容**：
- **敵人立繪**：位置 (200, 320)，200×200px，圓形遮罩，邊框 4px #FFD700
- **關卡名稱**：位置 (540, 50)，28px Bold #FFFFFF
- **敵人資訊面板**：位置 (450, 200)
  - HP血條：600×40px，背景 #333333，填充 #FF4444，圓角 20px
  - 屬性圖標：40×40px，位置 HP條左側
  - 倒數CD：120×120px圓形，背景 #FF8800，數字 48px Bold #FFFFFF
- **傷害Combo顯示區**：位置 (450, 400)
  - 本回合傷害預覽：24px Bold
  - 當前Combo數值：「連擊 x N」
  - 連擊倍率顯示：「倍率 x1.2」
- **技能按鈕區**：位置 (800, 300)，80×80px，預留英雄技能
...
**下層戰鬥操作區**：
- **候選圖塊區**：位置 (140, 1100)，4個槽位，每個160×160px
  - 顯示圖塊形狀與屬性
  - 支援拖曳到下方9×9棋盤
- **9×9實際戰鬥棋盤**：位置 (90, 1300)，900×900px
  - 背景色：#2E2E2E，邊框 4px #555555
  - 單格：94×94px，間距 4px，圓角 6px
  - 空格：背景 #F5F5F5，邊框 1px #DDDDDD
  - 障礙格：背景 #333333，斜線紋理
  - 高亮：可放置 #44FF4440，無效 #FF444440
- **操作按鈕**：
  - 送出按鈕：位置 (540, 1400)，280×80px，背景 #FF6600
  - 設定按鈕：位置 (920, 1450)，80×80px，背景 #666666

#### 4.2.4 結算畫面 (Result) 具體佈局
**上層結果展示區內容**：
- **勝利/失敗大字**：位置 (540, 100)，64px Bold #FFD700，文字陰影 4px
- **星級評價**：位置 (540, 200)，星星 80×80px，間距 20px
  - 金星 #FFD700，銀星 #C0C0C0，空星 #666666
- **戰鬥統計面板**：位置 (540, 350)，背景 #F8F8F8，圓角 20px
  - 總回合數 vs 目標回合數
  - 最大連擊數
  - 總傷害輸出
  - 剩餘HP（勝利時）
  - 完成時間
- **獎勵展示**：位置 (540, 550)，背景 #FFFFFF80，圓角 15px
  - 獲得金幣、經驗、道具等
...
**下層行動圖塊選擇區**：
- **四個行動圖塊**：水平排列，每個160×160px
  - 重試圖塊：位置 (190, 1200)，橙色 #FF8800
  - 繼續圖塊：位置 (370, 1200)，綠色 #44AA44（勝利時顯示）
  - 返回圖塊：位置 (550, 1200)，藍色 #4488FF
  - 分享圖塊：位置 (730, 1200)，紫色 #AA44FF（預留功能）
- **返回主選單按鈕**：位置 (80, 1750)，120×80px
...
1. **統一三層架構**：
   - 上層資訊顯示區：640px高，顯示各場景相關資訊
   - 中層統一九宮格：360×360px，所有場景使用相同規格
   - 下層圖塊選擇區：880px高，適應不同場景需求
2. **通用拖放系統**：
...
- [ ] 所有場景都使用統一的三層結構
- [ ] 上層資訊顯示區高度統一為640px
- [ ] 中層九宮格尺寸統一為360×360px，位置固定在(360, 640)
- [ ] 下層圖塊選擇區位置統一在(0, 1040)
- [ ] 九宮格單格規格統一：120×120px，無間距
- [ ] 所有場景的九宮格視覺效果一致
...
### 7.4 戰鬥系統測試
- [ ] 戰鬥圖塊可正常拖放到9×9實際棋盤
- [ ] 中層3×3九宮格顯示回合預覽而非棋盤操作
- [ ] 敵人資訊顯示區域足夠大，包含技能按鈕位置
- [ ] 傷害和Combo資訊顯示在敵人資訊區
- [ ] 棋盤放置邏輯和3×3區域識別正確
- [ ] 送出按鈕和設定按鈕位置合理
