...
#### 中層 - 統一互動確認區 (0, 1000) - (1080, 1300)  
**中等大小，統一的3×3互動區域**：
- 九宮格尺寸：300×300px，位置居中 (390, 1000)
- 單格尺寸：100×100px，間距：0px
- 中央確認格：特殊高亮效果
- 功能：確認操作、顯示選中項目預覽
- **注意**：戰鬥的3×3棋盤在上層，這裡是確認區

#### 下層 - 選項圖塊區 (0, 1300) - (1080, 1920)
**最小區域，提供選項和簡單描述**：
- **標準圖塊**：160×160px，簡潔設計
...
│   │   └── VersionLabel (版本資訊)
│   └── UnifiedConfirmGrid (中層統一確認區 300x300px)
│       ├── GridContainer (3x3, 100x100px單格)
│       ├── CenterSlot (目標放置區)
│       ├── GridHighlight (高亮效果)
│       └── ConfirmButton (確認按鈕，條件顯示)
├── CanvasLayer (互動層)
│   └── OptionsArea (下層選項區 620px高)
...
- **傷害分解顯示區**：位置 (450, 350)
  ...
  - 連擊倍率顯示：「倍率 x1.2」
  - 屬性加成顯示
- **3×3戰鬥棋盤**：位置 (390, 600)，300×300px
  - 實際的遊戲棋盤，每格100×100px
  - 障礙格標示、放置高亮
  - 已放置圖塊顯示
  - 這是真正的戰鬥區域，與其他介面的九宮格相同

#### 2.3.2 中層 - 操作確認區 (390, 1000) - (690, 1300)
**統一的確認操作區域**：
- **3×3 確認網格**：300×300px，與其他介面相同規格
- **單格尺寸**：100×100px，無間距
- **確認區域功能**：顯示"送出本回合"確認
- **操作預覽**：可顯示當前回合操作的縮略圖

#### 2.3.3 下層 - 候選圖塊與控制區 (0, 1300) - (1080, 1920)
**最小區域，候選圖塊和控制按鈕**：
- **候選圖塊區**：4個圖塊槽位，每個 160×160px，位置 (140, 1350)
  - 顯示圖塊形狀與屬性
  - 支援拖曳到上層的3×3戰鬥棋盤
  - 使用後立即補充新圖塊
- **送出按鈕**：位置 (540, 1550)，200×60px
- **設定按鈕**：位置 (880, 1550)，80×60px
...
- **中層統一互動確認區**：位置 (390, 1000)，尺寸 300×300px（中等）
- **下層選項圖塊區**：位置 (0, 1300)，尺寸 1080×620px（最小）
...
#### 4.1.3 中層統一互動確認區規格 (300×300px)
**所有介面使用相同規格**：
- **位置**：(390, 1000) 固定位置，水平居中
- **總尺寸**：300×300px
- **網格配置**：3×3，每格 100×100px
- **格子間距**：0px（緊密排列）
- **背景**：半透明黑色 (#000000AA)
- **圓角**：15px
- **邊框**：2px 實線 #444444
...
**下層功能圖塊排列 (620px高)**：
- **四個圖塊水平居中排列**：
  - 戰鬥圖塊：(190, 1350)，背景色 #FF4444
  - 構築圖塊：(370, 1350)，背景色 #4488FF
  - 商店圖塊：(550, 1350)，背景色 #FFD700
  - 設定圖塊：(730, 1350)，背景色 #888888
...
**下層關卡圖塊滾動區 (620px高)**：
- **滾動容器**：位置 (40, 1350)，1000×400px
- **可視區域**：1000×200px，支援水平滾動和右滑查看
- **關卡圖塊規格**：
  - 尺寸：160×160px（統一標準）
  - 間距：20px
...
- **星級評價**：位置 (540, 250)，星星 100×100px，間距 30px
...
- **四個行動圖塊**：水平排列，每個160×160px
  - 重試圖塊：位置 (190, 1350)，橙色 #FF8800
  - 繼續圖塊：位置 (370, 1350)，綠色 #44AA44（勝利時顯示）
  - 返回圖塊：位置 (550, 1350)，藍色 #4488FF
  - 分享圖塊：位置 (730, 1350)，紫色 #AA44FF（預留功能）
- **返回主選單按鈕**：位置 (80, 1800)，150×80px
...
- [ ] 中層統一確認區尺寸統一為300×300px，位置固定在(390, 1000)
- [ ] 下層選項區位置統一在(0, 1300)，高度620px（最小）
- [ ] 所有九宮格單格規格統一：100×100px，無間距
- [ ] 所有場景的確認區視覺效果一致
- [ ] 戰鬥場景的3×3棋盤在上層，確認區在中層，候選圖塊在下層
...
### 7.4 戰鬥系統測試
- [ ] 戰鬥圖塊可正常拖放到上層的3×3戰鬥棋盤
- [ ] 中層確認區顯示"送出本回合"確認而非棋盤操作
- [ ] 上層區域足夠大，完整顯示敵人資訊、技能按鈕、傷害分解和3×3棋盤
- [ ] 傷害分解、Combo、技能按鈕都在上層合理分佈
- [ ] 3×3棋盤放置邏輯正確，與其他介面的九宮格一致
- [ ] 下層候選圖塊和控制按鈕位置合理，不佔用過多空間
